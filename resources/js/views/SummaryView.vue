...
/**
 * Resumen del día: los números de la granja de un vistazo.
 *
 * Vivían en el inicio, encima de los botones. El inicio se abre para registrar
 * algo, y las tarjetas empujaban las acciones fuera de la primera pantalla.
 * Aquí tienen sitio propio y el inicio queda limpio.
 *
 * No se confunde con Reportes: esto es "cómo va la granja ahora mismo";
 * Reportes es el análisis de un periodo que tú eliges.
 */
import { ref, onMounted, computed, watch } from 'vue'
import ScreenShell from '@/components/ui/ScreenShell.vue'
import PenSelector from '@/components/ui/PenSelector.vue'
import { useFarmStore } from '@/stores/farm'
import { todayEggs, aliveChickens, pendingDebt, nextVaccine, overdueVaccine } from '@/stores/sync'
...
const vaccineLabel = computed(() => {
  if (overdue.value?.nextAt) {
    const days = Math.ceil(
      (startOfFarmDay().getTime() - new Date(overdue.value.nextAt).getTime()) / 86_400_000,
    )
...
async function refresh() {
  ...
  ;[eggsToday.value, alive.value, debt.value, vaccine.value, overdue.value] = await Promise.all([
...
<template>
  <ScreenShell title="Resumen" :back="false">
    ...
    <section class="grid grid-cols-2 gap-3">
      <div class="card">
        <p class="text-sm font-semibold text-slate-500">
          ...
          <span v-else-if="farm.activePens.length > 1" class="ml-1 text-xs text-slate-400">
            · Todos
          </span>
        </p>
      ...
      <div class="card col-span-2">
        ...
        <p class="num-big" :class="(margin?.iofc ?? 0) >= 0 ? 'text-grass-600' : 'text-alert-600'">
